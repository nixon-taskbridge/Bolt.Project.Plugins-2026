...
{
    public class GenCostPriceCalculations : IPlugin
    {
        /// <summary>
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            if (context.MessageName == "Delete")
            {
                try
                {
                    ...
                    CalculateRollupFieldRequest request = new CalculateRollupFieldRequest
                    {
                        ...
                        FieldName = "bolt_totalgenrentalprice"
                    };
                ...
                {
                    throw new InvalidPluginExecutionException("An error occurred in Delete step of Rental Generator Price Calculation plugin.", ex);
                }

                catch (Exception ex)
                {
                    tracingService.Trace("Delete step Rental Generator Price Calculation plugin: {0}", ex.ToString());
                    throw;
                }
            }
                ...
                context.InputParameters["Target"] is Entity)
            {
                ...
                try
                {
                    ...
                    // Interval
                    int interval_Value;
                    if (entity.Attributes.Contains("bolt_rentaltypegen"))
                    {
                        OptionSetValue rental_Interval = entity.GetAttributeValue<OptionSetValue>("bolt_rentaltypegen");
                        interval_Value = rental_Interval.Value;
                    }
                    else
                    {
                        OptionSetValue rental_Interval = pre_entity.GetAttributeValue<OptionSetValue>("bolt_rentaltypegen");
                        interval_Value = rental_Interval.Value;
                    }

                    // Shift
                    int shift_Value;
                    if (entity.Attributes.Contains("bolt_rentalshiftgen"))
                    {
                        OptionSetValue rental_Shift = entity.GetAttributeValue<OptionSetValue>("bolt_rentalshiftgen");
                        shift_Value = rental_Shift.Value;
                    }
                    else
                    {
                        OptionSetValue rental_Shift = pre_entity.GetAttributeValue<OptionSetValue>("bolt_rentalshiftgen");
                        shift_Value = rental_Shift.Value;
                    }
                    var shift_multipliers = new Dictionary<int, decimal>()
                    {
                        {454890000, (decimal)1.0}, // Single
                        {454890003, (decimal)1.0}, // Standby
                    ...
                    decimal daily_cost_value = daily_input_value * shift_multipliers[shift_Value];
                    decimal weekly_cost_value = weekly_input_value * shift_multipliers[shift_Value];
                    decimal monthly_cost_value = monthly_input_value * shift_multipliers[shift_Value];
                ...
                {
                    throw new InvalidPluginExecutionException("An error occurred in Rental Generator Price Calculation plugin.", ex);
                }

                catch (Exception ex)
                {
                    tracingService.Trace("Rental Generator Price Calculation plugin: {0}", ex.ToString());
                    throw;
                }
            }
        }
