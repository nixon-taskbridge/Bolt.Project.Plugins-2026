...
{
    public class ResiCostsheet_TotalAddersCalculation : IPlugin
    {
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
                    ...
                    try
                    {
                        if (context.PreEntityImages.Contains("Image"))
                        {
                            ...
                            if (!preImage.Attributes.Contains("quote"))
                                return;
                            relatedQuote_guid = (preImage.GetAttributeValue<EntityReference>("bolt_residentialcostsheet")).Id;
                        ...
                        else if (context.PostEntityImages.Contains("Image"))
                        {
                            ...
                            if (!postImage.Attributes.Contains("bolt_residentialcostsheet"))
                                return;
                            relatedQuote_guid = (postImage.GetAttributeValue<EntityReference>("bolt_residentialcostsheet")).Id;
                            Calculate_TotalAdder_Cost(relatedQuote_guid);
                        }
                    }
                    catch (Exception ex)
            ...
            else if (context.MessageName == "Delete")
            {
                if (context.PreEntityImages.Contains("Image"))
                {
                    ...
                    if (!preImageInvoice.Attributes.Contains("bolt_residentialcostsheet"))
                        return;
                    relatedQuote_guid = (preImageInvoice.GetAttributeValue<EntityReference>("bolt_residentialcostsheet")).Id;
        ...
        private void Calculate_TotalAdder_Cost(Guid QuoteID)
        {
            ...
            query.Criteria.AddCondition("bolt_residentialcostsheet", ConditionOperator.Equal, query_quote);
            ...
            Entity resiquote = new Entity("bolt_residentialcostsheet");
